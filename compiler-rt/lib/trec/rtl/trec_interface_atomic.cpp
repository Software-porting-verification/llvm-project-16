...
template <typename T>
T func_xchg(volatile T *v, T op, uptr pc, bool isPtr,
...
template <typename T>
T func_add(volatile T *v, T op, uptr pc, bool isPtr, __sanitizer::u64 debugID) {
...
template <typename T>
T func_sub(volatile T *v, T op, uptr pc, bool isPtr, __sanitizer::u64 debugID) {
...
template <typename T>
T func_and(volatile T *v, T op, uptr pc, bool isPtr, __sanitizer::u64 debugID) {
...
template <typename T>
T func_or(volatile T *v, T op, uptr pc, bool isPtr, __sanitizer::u64 debugID) {
...
template <typename T>
T func_xor(volatile T *v, T op, uptr pc, bool isPtr, __sanitizer::u64 debugID) {
...
template <typename T>
T func_nand(volatile T *v, T op, uptr pc, bool isPtr,
            __sanitizer::u64 debugID) {
...
template <typename T>
T func_cas(volatile T *v, T cmp, T xch, uptr pc, bool isPtr,
           __sanitizer::u64 debugID) {
...
static T AtomicLoad(ThreadState *thr, uptr pc, const volatile T *a, morder mo,
                    bool isPtr, __sanitizer::u64 debugID) {
  ...
  if (!IsAcquireOrder(mo)) {
    ...
    MemoryReadAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)v,
                     {1, 0, 0, 1}, debugID);
    FuncExitParam(cur_thread(), {0, 1, 0, (uptr)a}, v, debugID);
    return v;
  }
  // Don't create sync object if it does not exist yet. For example, an atomic
  // pointer is initialized to nullptr and then periodically acquire-loaded.
  T v = NoTrecAtomicLoad(a, mo);
  MemoryReadAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)v, {1, 0, 0, 1},
                   debugID);
  FuncExitParam(cur_thread(), {0, 1, 0, (uptr)a}, v, debugID);
  return v;
}
...
static void AtomicStore(ThreadState *thr, uptr pc, volatile T *a, T v,
                        morder mo, bool isPtr, __sanitizer::u64 debugID) {
...
template <typename T, T (*F)(volatile T *v, T op, uptr pc, bool isPtr,
                             __sanitizer::u64 debugID)>
static T AtomicRMW(ThreadState *thr, uptr pc, volatile T *a, T v, morder mo,
                   bool isPtr, __sanitizer::u64 debugID) {
  v = F(a, v, pc, isPtr, debugID);
  return v;
}

template <typename T>
static T NoTrecAtomicExchange(volatile T *a, T v, morder mo) {
  return func_xchg(a, v, GET_CALLER_PC(), false, 0);
}

template <typename T>
static T NoTrecAtomicFetchAdd(volatile T *a, T v, morder mo) {
  return func_add(a, v, GET_CALLER_PC(), false, 0);
}

template <typename T>
static T NoTrecAtomicFetchSub(volatile T *a, T v, morder mo) {
  return func_sub(a, v, GET_CALLER_PC(), false, 0);
}

template <typename T>
static T NoTrecAtomicFetchAnd(volatile T *a, T v, morder mo) {
  return func_and(a, v, GET_CALLER_PC(), false, 0);
}

template <typename T>
static T NoTrecAtomicFetchOr(volatile T *a, T v, morder mo) {
  return func_or(a, v, GET_CALLER_PC(), false, 0);
}

template <typename T>
static T NoTrecAtomicFetchXor(volatile T *a, T v, morder mo) {
  return func_xor(a, v, GET_CALLER_PC(), false, 0);
}

template <typename T>
static T NoTrecAtomicFetchNand(volatile T *a, T v, morder mo) {
  return func_nand(a, v, GET_CALLER_PC(), false, 0);
}

template <typename T>
static T AtomicExchange(ThreadState *thr, uptr pc, volatile T *a, T v,
                        morder mo, bool isPtr, __sanitizer::u64 debugID) {
  T res = AtomicRMW<T, func_xchg>(thr, pc, a, v, mo, isPtr, debugID);
  return res;
}
...
static T AtomicFetchAdd(ThreadState *thr, uptr pc, volatile T *a, T v,
                        morder mo, bool isPtr, __sanitizer::u64 debugID) {
  T res = AtomicRMW<T, func_add>(thr, pc, a, v, mo, isPtr, debugID);
  return res;
}
...
static T AtomicFetchSub(ThreadState *thr, uptr pc, volatile T *a, T v,
                        morder mo, bool isPtr, __sanitizer::u64 debugID) {
  T res = AtomicRMW<T, func_sub>(thr, pc, a, v, mo, isPtr, debugID);
  return res;
}
...
static T AtomicFetchAnd(ThreadState *thr, uptr pc, volatile T *a, T v,
                        morder mo, bool isPtr, __sanitizer::u64 debugID) {
  T res = AtomicRMW<T, func_and>(thr, pc, a, v, mo, isPtr, debugID);
  return res;
}
...
static T AtomicFetchOr(ThreadState *thr, uptr pc, volatile T *a, T v, morder mo,
                       bool isPtr, __sanitizer::u64 debugID) {
  T res = AtomicRMW<T, func_or>(thr, pc, a, v, mo, isPtr, debugID);
  return res;
}
...
static T AtomicFetchXor(ThreadState *thr, uptr pc, volatile T *a, T v,
                        morder mo, bool isPtr, __sanitizer::u64 debugID) {
  T res = AtomicRMW<T, func_xor>(thr, pc, a, v, mo, isPtr, debugID);
  return res;
}
...
static T AtomicFetchNand(ThreadState *thr, uptr pc, volatile T *a, T v,
                         morder mo, bool isPtr, __sanitizer::u64 debugID) {
  T res = AtomicRMW<T, func_nand>(thr, pc, a, v, mo, isPtr, debugID);
  return res;
}
...
static bool AtomicCAS(ThreadState *thr, uptr pc, volatile T *a, T *c, T v,
  ...
  T cc = *c;
  T pr = func_cas(a, cc, v, pc, isPtr, debugID);
  if (pr == cc) {
    return true;
  }
  *c = pr;
...
static T AtomicCAS(ThreadState *thr, uptr pc, volatile T *a, T c, T v,
                   morder mo, morder fmo, bool isPtr,
                   __sanitizer::u64 debugID) {
  AtomicCAS(thr, pc, a, &c, v, mo, fmo, isPtr, debugID);
  return c;
}
...
#define SCOPED_ATOMIC_TREC(func, isPtr, debugID, ...)            \
  ThreadState *const thr = cur_thread();                         \
  if (UNLIKELY(thr->ignore_sync || thr->ignore_interceptors)) {  \
    ProcessPendingSignals(thr);                                  \
    return NoTrecAtomic##func(__VA_ARGS__);                      \
  ...
  mo = convert_morder(mo);                                       \
  ScopedAtomic sa(thr, callpc, a, mo, __func__);                 \
  return Atomic##func(thr, callpc, __VA_ARGS__, isPtr, debugID); \
  /**/
...
a8 __trec_atomic8_load(const volatile a8 *a, morder mo, bool isPtr,
                       __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(Load, isPtr, debugID, a, mo);
}
...
void __trec_atomic8_store(volatile a8 *a, a8 v, morder mo, bool isPtr,
                          __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(Store, isPtr, debugID, a, v, mo);
}
...
a8 __trec_atomic8_exchange(volatile a8 *a, a8 v, morder mo, bool isPtr,
                           __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(Exchange, isPtr, debugID, a, v, mo);
}
...
a8 __trec_atomic8_fetch_add(volatile a8 *a, a8 v, morder mo, bool isPtr,
                            __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(FetchAdd, isPtr, debugID, a, v, mo);
}
...
a8 __trec_atomic8_fetch_sub(volatile a8 *a, a8 v, morder mo, bool isPtr,
                            __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(FetchSub, isPtr, debugID, a, v, mo);
}
...
a8 __trec_atomic8_fetch_and(volatile a8 *a, a8 v, morder mo, bool isPtr,
                            __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(FetchAnd, isPtr, debugID, a, v, mo);
}
...
a8 __trec_atomic8_fetch_or(volatile a8 *a, a8 v, morder mo, bool isPtr,
                           __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(FetchOr, isPtr, debugID, a, v, mo);
}
...
a8 __trec_atomic8_fetch_xor(volatile a8 *a, a8 v, morder mo, bool isPtr,
                            __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(FetchXor, isPtr, debugID, a, v, mo);
}
...
a8 __trec_atomic8_fetch_nand(volatile a8 *a, a8 v, morder mo, bool isPtr,
                             __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(FetchNand, isPtr, debugID, a, v, mo);
}
...
int __trec_atomic8_compare_exchange_strong(volatile a8 *a, a8 *c, a8 v,
                                           morder mo, morder fmo, bool isPtr,
                                           __sanitizer::u64 debugID) {
  SCOPED_ATOMIC_TREC(CAS, isPtr, debugID, a, c, v, mo, fmo);
}
